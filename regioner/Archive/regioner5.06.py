...
#       self.output.postscript(file='test.ps', colormode='color', pagewidth=ww-1, pageheight=wh-1)
        ...
        # READ HERE
        # The painting scale issue is due to the cavas.postscript() method using 96 dpi as the default instead of 72 dpi, SO we have to scale the image.
        # self.output.postscript does not let me define the pic dimensions as exactly the canvas dims, so it is slightly undersized to prevent pixel loss.
        # https://stackoverflow.com/questions/53509131/tkinter-saving-canvas-at-wrong-scale
        # https://stackoverflow.com/questions/51742871/how-to-maintain-canvas-size-when-converting-python-turtle-canvas-to-bitmap?noredirect=1&lq=1
...
def binary_mask_cell_count(background_pil, sensitivity):
    logger.debug(f"Starting binary mask cell count with sensitivity: {sensitivity}")
